...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\alind\\Documents\\Neurotech\\psychopy\\strooptesting_control_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "fixation_control" ---
    # Run 'Begin Experiment' code from clock_setup
    newClock = core.Clock()
    dot_control = visual.TextStim(win=win, name='dot_control',
        text='+',
        font='Open Sans',
    ...
    # --- Prepare to start Routine "fixation_control" ---
    # create an object to store info about Routine fixation_control
    fixation_control = data.Routine(
        name='fixation_control',
        components=[dot_control],
    )
    fixation_control.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for fixation_control
    fixation_control.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    fixation_control.tStart = globalClock.getTime(format='float')
    fixation_control.status = STARTED
    thisExp.addData('fixation_control.started', fixation_control.tStart)
    fixation_control.maxDuration = 60
    win.color = [-1.0000, -1.0000, -1.0000]
    win.colorSpace = 'rgb'
    win.backgroundImage = ''
    win.backgroundFit = 'none'
    # keep track of which components have finished
    fixation_controlComponents = fixation_control.components
    for thisComponent in fixation_control.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "fixation_control" ---
    fixation_control.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 60.0:
        ...
        # update/draw components on each frame
        # is it time to end the Routine? (based on local clock)
        if tThisFlip > fixation_control.maxDuration-frameTolerance:
            fixation_control.maxDurationReached = True
            continueRoutine = False
        
        # *dot_control* updates
        
        # if dot_control is starting this frame...
        if dot_control.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            dot_control.frameNStart = frameN  # exact frame index
            dot_control.tStart = t  # local t and not account for scr refresh
            dot_control.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(dot_control, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'dot_control.started')
            # update status
            dot_control.status = STARTED
            dot_control.setAutoDraw(True)
        
        # if dot_control is active this frame...
        if dot_control.status == STARTED:
            # update params
            pass
        
        # if dot_control is stopping this frame...
        if dot_control.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 60-frameTolerance:
                # keep track of stop time/frame for later
                dot_control.tStop = t  # not accounting for scr refresh
                dot_control.tStopRefresh = tThisFlipGlobal  # on global time
                dot_control.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'dot_control.stopped')
                # update status
                dot_control.status = FINISHED
                dot_control.setAutoDraw(False)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            fixation_control.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in fixation_control.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "fixation_control" ---
    for thisComponent in fixation_control.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for fixation_control
    fixation_control.tStop = globalClock.getTime(format='float')
    fixation_control.tStopRefresh = tThisFlipGlobal
    thisExp.addData('fixation_control.stopped', fixation_control.tStop)
    setupWindow(expInfo=expInfo, win=win)
    # Run 'End Routine' code from clock_setup
    newClock.reset()
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if fixation_control.maxDurationReached:
        routineTimer.addTime(-fixation_control.maxDuration)
    elif fixation_control.forceEnded:
        routineTimer.reset()
    else:
    ...
    for thisStroop in stroop:
        ...
        trial = data.Routine(
            name='trial',
            components=[press, dot, text_stroop],
        )
